...
def run_identifier_for_advertising_unsupported_device_ui_absence_check(dab_topic, test_name, tester, device_id):
    ...
    Logic:
      - Inspect supported settings via EnforcementManager.get_supported_settings() to see if
        identifierForAdvertising is declared in DAB.
    ...
    try:
        # Header / description
        for line in (
            "[TEST] IdentifierForAdvertising absence on unsupported devices (UI check)",
            "[DESC] Verify that when identifierForAdvertising is NOT declared in DAB, the settings UI does not display an Advertising ID.",
        ):
            LOGGER.result(line)
            logs.append(line)

        # Step 1: Fetch supported settings via EnforcementManager and inspect identifierForAdvertising
        line = "[STEP] Fetching supported settings from EnforcementManager to see if identifierForAdvertising is declared."
        LOGGER.result(line)
        logs.append(line)
        ...
        try:
            ...
            sup_dict = {}
            info_line = f"[INFO] Could not parse supported settings map from EnforcementManager: {e}; falling back to manual UI-only check."
            LOGGER.result(info_line)
            logs.append(info_line)
        ...
        line = "[STEP] Manual verification – ensure no Advertising ID is visible anywhere in settings UI."
        LOGGER.result(line)
        logs.append(line)
        ...
        result.test_result = "OPTIONAL_FAILED"
        line = (
            f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' not supported while running "
            "identifierForAdvertising unsupported-device UI absence check."
        )
        LOGGER.result(line)
        logs.append(line)
        ...
        result.test_result = "SKIPPED"
        line = (
            f"[RESULT] SKIPPED — Internal error during identifierForAdvertising unsupported-device UI absence check: {e}"
        )
        LOGGER.result(line)
        logs.append(line)
        ...
        summary_line = (
            f"[SUMMARY] outcome={result.test_result}, user_saw_ad_id={user_saw_ad_id}, "
            f"test_id={test_id}, device={device_id}"
        )
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_high_contrast_text_invalid_value_type_check(dab_topic, test_name, tester, device_id):
    ...
    # Header / description
    for line in (
        "[TEST] highContrastText invalid value type (string instead of boolean)",
        "[DESC] Send system/settings/set with highContrastText=\"true\" (string) and expect 400 + no change in the setting.",
    ):
        LOGGER.result(line)
        logs.append(line)
    ...
    try:
        ...
        line = "[STEP] Reading current highContrastText value via system/settings/get."
        LOGGER.result(line)
        logs.append(line)

        status_before, resp_before = execute_cmd_and_log(
            tester, device_id, "system/settings/get", "{}", logs, result
        )
        if status_before != 200:
            result.test_result = "SKIPPED"
            line = f"[RESULT] SKIPPED — system/settings/get before negative set failed with status={status_before}; cannot establish baseline."
            LOGGER.result(line)
            logs.append(line)
        ...
        try:
            before_obj = json.loads(resp_before) if isinstance(resp_before, str) else (resp_before or {})
        except Exception as e:
            result.test_result = "SKIPPED"
            line = f"[RESULT] SKIPPED — system/settings/get returned invalid JSON when reading baseline highContrastText: {e}"
            LOGGER.result(line)
            logs.append(line)
            summary_line = f"[SUMMARY] {test_name} — final result: SKIPPED, test_id={test_id}, device={device_id}"
            LOGGER.result(summary_line)
        ...
        original_value = before_obj.get("highContrastText", None)
        line = f"[INFO] Baseline highContrastText value: {original_value!r}"
        LOGGER.result(line)
        logs.append(line)

        # Step 3: Send invalid value (string instead of boolean)
        invalid_payload = json.dumps({"highContrastText": "string_instead_of_boolean"})
        line = f"[STEP] Sending invalid highContrastText value via system/settings/set with payload: {invalid_payload}"
        LOGGER.result(line)
        logs.append(line)

        status_set, resp_set = execute_cmd_and_log(
            tester, device_id, "system/settings/set", invalid_payload, logs, result
        )

        if status_set == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/settings/set returned 501; highContrastText not implemented as a writable setting on this device."
            LOGGER.result(line)
            logs.append(line)
            summary_line = f"[SUMMARY] {test_name} — final result: OPTIONAL_FAILED, test_id={test_id}, device={device_id}"
            LOGGER.result(summary_line)
        ...
        if status_set != 400:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Expected status 400 for invalid highContrastText value type, but got status={status_set}."
            LOGGER.result(line)
            logs.append(line)
            summary_line = f"[SUMMARY] {test_name} — final result: FAILED, test_id={test_id}, device={device_id}"
            LOGGER.result(summary_line)
        ...
        line = "[INFO] Device returned status 400 for invalid highContrastText payload as expected."
        LOGGER.result(line)
        logs.append(line)

        # Step 4: Re-read setting and confirm it did not change
        line = "[STEP] Re-reading highContrastText via system/settings/get to confirm the value is unchanged."
        LOGGER.result(line)
        logs.append(line)

        status_after, resp_after = execute_cmd_and_log(
            tester, device_id, "system/settings/get", "{}", logs, result
        )
        if status_after != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get after invalid set failed with status={status_after}; cannot confirm value unchanged."
            LOGGER.result(line)
            logs.append(line)
            summary_line = f"[SUMMARY] {test_name} — final result: FAILED, test_id={test_id}, device={device_id}"
            LOGGER.result(summary_line)
        ...
        try:
            after_obj = json.loads(resp_after) if isinstance(resp_after, str) else (resp_after or {})
        except Exception as e:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get returned invalid JSON after invalid set: {e}"
            LOGGER.result(line)
            logs.append(line)
            summary_line = f"[SUMMARY] {test_name} — final result: FAILED, test_id={test_id}, device={device_id}"
            LOGGER.result(summary_line)
        ...
        line = f"[INFO] highContrastText after invalid set attempt: {value_after!r}"
        LOGGER.result(line)
        logs.append(line)
        ...
        if value_after != original_value:
            result.test_result = "FAILED"
            line = (
                f"[RESULT] FAILED — highContrastText changed from {original_value!r} to {value_after!r} "
                "even though system/settings/set returned 400 for invalid type."
            )
            LOGGER.result(line)
            logs.append(line)
        else:
            result.test_result = "PASS"
            line = (
                "[RESULT] PASS — Device rejected string highContrastText value with status=400 "
                "and preserved the previous boolean value."
            )
            LOGGER.result(line)
            logs.append(line)
        ...
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' not supported while running highContrastText invalid-type test."
        LOGGER.result(line)
        logs.append(line)
        summary_line = f"[SUMMARY] {test_name} — final result: OPTIONAL_FAILED, test_id={test_id}, device={device_id}"
        LOGGER.result(summary_line)
        ...
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — Internal error during highContrastText invalid-value-type test: {e}"
        LOGGER.result(line)
        logs.append(line)
        summary_line = f"[SUMMARY] {test_name} — final result: SKIPPED, test_id={test_id}, device={device_id}"
        LOGGER.result(summary_line)
...
def run_logs_stop_without_active_collection_check(dab_topic, test_name, tester, device_id):
    ...
    try:
        # Header / description
        for line in (
            "[TEST] Logs Stop-Collection without active collection",
            "[DESC] Verify that system/logs/stop-collection returns a 'not active' style error when no log collection is running.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        if not require_capabilities(tester, device_id, cap_spec, result, logs):
            # require_capabilities already set OPTIONAL_FAILED and logged details.
            return result

        # Step 2: First stop-collection (normalization / cleanup)
        line = "[STEP] Sending initial system/logs/stop-collection to ensure no active log collection remains."
        LOGGER.result(line)
        logs.append(line)

        status1, resp1 = execute_cmd_and_log(
            tester, device_id, "system/logs/stop-collection", "{}", logs, result
        )
        info1 = f"[INFO] First stop-collection returned status={status1}; response={resp1}"
        LOGGER.result(info1)
        logs.append(info1)

        # No assertion on status1; we only care about the second call.

        # Step 3: Second stop-collection (actual assertion)
        line = "[STEP] Sending second system/logs/stop-collection to validate behavior when collection is not active."
        LOGGER.result(line)
        logs.append(line)

        status2, resp2 = execute_cmd_and_log(
            tester, device_id, "system/logs/stop-collection", "{}", logs, result
        )
        ...
        if status2 == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = (
                "[RESULT] OPTIONAL_FAILED — system/logs/stop-collection returned 501; "
                "log collection stop is not implemented on this device."
            )
            LOGGER.result(line)
            logs.append(line)
            ...
            result.test_result = "PASS"
            line = (
                "[RESULT] PASS — Second system/logs/stop-collection returned status 400, "
                "indicating log collection was not active as expected."
            )
            LOGGER.result(line)
            logs.append(line)
            ...
            result.test_result = "FAILED"
            line = (
                "[RESULT] FAILED — Second system/logs/stop-collection returned status 200 even though logging "
                "should already be stopped; device did not expose a 'log collection not active' error."
            )
            LOGGER.result(line)
            logs.append(line)
            ...
            result.test_result = "FAILED"
            line = (
                f"[RESULT] FAILED — Second system/logs/stop-collection returned unexpected status {status2}; "
        ...
        line = (
            f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' not supported while running "
            "logs stop-without-active collection test."
        )
        LOGGER.result(line)
        logs.append(line)
        ...
        result.test_result = "SKIPPED"
        line = (
            f"[RESULT] SKIPPED — Internal error during logs stop-without-active collection test: {e}"
        )
        LOGGER.result(line)
        logs.append(line)
    ...
    # Final summary line
    summary_line = (
        f"[SUMMARY] {test_name} — final result: {result.test_result}, "
        f"test_id={test_id}, device={device_id}"
    )
    LOGGER.result(summary_line)
    logs.append(summary_line)
...
def run_brightness_mid_level_50_screen_check(dab_topic, test_name, tester, device_id):
    """
    Validates that setting brightness to a mid-level (~50%) value via DAB is stored correctly
    and reflected on the screen. This is a manual-assisted test.
    """
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    # Use main operation as topic, same as screensaver tests
    result = TestResult(test_id, device_id, "system/settings/set", "{}", "UNKNOWN", "", logs)
    user_validated = "N/A"
    original_brightness = None
    min_brightness = None
    ...
    try:
        # Header and description
        for line in (
            f"[TEST] Brightness Mid-Level (~50%) Screen Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Set brightness to a mid-level (~50%) value and verify it is reflected on the screen.",
            "[DESC] Required operations: system/settings/get, system/settings/set.",
            "[DESC] Pass criteria: Device reports mid-level brightness via DAB and user confirms mid-level brightness visually.",
        ):
            LOGGER.result(line)
            logs.append(line)

        # Capability gate
        if not require_capabilities(tester, device_id, "ops: system/settings/get, system/settings/set | settings: brightness", result, logs,):
            return result

        # Step 1: Resolve brightness range via EnforcementManager (settings/list)
        line = "[STEP] Resolving brightness range using EnforcementManager settings descriptor."
        LOGGER.result(line)
        logs.append(line)
        ...
        if not isinstance(desc, dict) or not {"min", "max"}.issubset(desc.keys()):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness descriptor does not expose numeric min/max; cannot compute mid-level."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        max_brightness = desc["max"]
        LOGGER.result(f"[INFO] brightness range from settings/list: min={min_brightness!r}, max={max_brightness!r}")
        logs.append(f"[INFO] brightness range from settings/list: min={min_brightness!r}, max={max_brightness!r}")

        if not isinstance(min_brightness, (int, float)) or not isinstance(max_brightness, (int, float)):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness min/max values are not numeric; cannot compute mid-level target."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            mid_brightness = min_brightness if span <= 0 else int(round((min_brightness + max_brightness) / 2.0))
        except Exception:
            mid_brightness = min_brightness
        line = f"[INFO] Mid-level brightness selected as ~50% of range: {mid_brightness!r}."
        LOGGER.result(line)
        logs.append(line)

        # Step 2: Read current brightness (for restore)
        line = "[STEP] Fetching current brightness value via system/settings/get."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get failed with status {status}; cannot read current brightness."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get returned invalid JSON when reading original brightness: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        original_brightness = body_get1.get("brightness")
        line = f"[INFO] Original brightness value from device: {original_brightness!r}"
        LOGGER.result(line)
        logs.append(line)

        # Step 3: Set brightness to mid-level
        payload_mid = json.dumps({"brightness": mid_brightness})
        line = f"[STEP] Setting brightness to mid-level (~50%) with payload: {payload_mid}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_mid, logs, result)
        status = dab_status_from(response, rc)
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/settings/set for brightness is not implemented on this device."
            LOGGER.result(line)
            logs.append(line)
            return result
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/set to mid-level returned status {status}; cannot proceed."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 4: Confirm brightness is at mid-level
        line = "[STEP] Confirming brightness is at mid-level via system/settings/get."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get after setting mid-level failed with status {status}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get returned invalid JSON after setting mid-level: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        confirmed_brightness = body_get2.get("brightness")
        line = f"[INFO] Brightness after setting to mid-level: {confirmed_brightness!r} (target={mid_brightness!r})"
        LOGGER.result(line)
        logs.append(line)

        if confirmed_brightness != mid_brightness:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Expected brightness {mid_brightness!r}, observed {confirmed_brightness!r}."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 5: Manual visual confirmation on screen
        line = "[STEP] Manual check required — observe the screen at the mid-level brightness setting."
        LOGGER.result(line)
        logs.append(line)

        line = "[DESC] Compare this mid-level brightness against known minimum and maximum levels."
        LOGGER.result(line)
        logs.append(line)

        user_validated = yes_or_no(result, logs, "Does the screen brightness appear at a reasonable mid-level (~50%) compared to minimum and maximum?")
        if user_validated:
            result.test_result = "PASS"
            line = "[RESULT] PASS — User confirmed the screen shows an appropriate mid-level brightness."
        else:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — User reported the screen does NOT appear at a reasonable mid-level brightness."
        ...
        # Best-effort restore of original brightness
        try:
            if original_brightness is not None:
                payload_restore = json.dumps({"brightness": original_brightness})
                line = f"[STEP] Restoring original brightness value with payload: {payload_restore}"
                LOGGER.result(line)
                logs.append(line)
                rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_restore, logs, result)
                line = f"[INFO] Restore brightness status={dab_status_from(response, rc)}"
                LOGGER.result(line)
                logs.append(line)
        except Exception as e:
            line = f"[INFO] Failed to restore original brightness value: {e}"
            LOGGER.result(line)
            logs.append(line)

        # Final summary log
        line = f"[SUMMARY] outcome={result.test_result}, user_validated_mid_level={user_validated}, test_id={test_id}, device={device_id}"
        LOGGER.result(line)
        logs.append(line)
...
def run_brightness_60_video_content_check(dab_topic, test_name, tester, device_id):
    """
    Validates that setting brightness to a ~60% value via DAB is stored correctly
    and reflected while video content is playing. This is a manual-assisted test.
    """
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    result = TestResult(test_id, device_id, "system/settings/set", "{}", "UNKNOWN", "", logs)
    ...
    try:
        # Header and description
        for line in (
            f"[TEST] Brightness 60% Video Content Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Set brightness to a ~60% value and verify it is reflected correctly while video content is playing.",
            "[DESC] Required operations: system/settings/get, system/settings/set.",
            "[DESC] Pass criteria: Device reports ~60% brightness via DAB and user confirms video brightness matches.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        if not isinstance(desc, dict) or not {"min", "max"}.issubset(desc.keys()):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness descriptor does not expose numeric min/max; cannot compute ~60% value."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if not isinstance(min_brightness, (int, float)) or not isinstance(max_brightness, (int, float)):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness min/max values are not numeric; cannot compute ~60% target."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            span = max_brightness - min_brightness
            target_brightness = min_brightness if span <= 0 else int(round(min_brightness + 0.6 * span))
        except Exception:
            target_brightness = min_brightness
        line = f"[INFO] Target brightness selected as ~60% of range: {target_brightness!r}."
        LOGGER.result(line)
        logs.append(line)
        ...
        # Step 3: Set brightness to ~60%
        payload_target = json.dumps({"brightness": target_brightness})
        line = f"[STEP] Setting brightness to ~60% value with payload: {payload_target}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_target, logs, result)
        status = dab_status_from(response, rc)
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/settings/set for brightness is not implemented on this device."
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/set to ~60% returned status {status}; cannot proceed."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 4: Confirm brightness is at ~60% via get
        line = "[STEP] Confirming brightness is at ~60% via system/settings/get."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get after setting ~60% failed with status {status}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get returned invalid JSON after setting ~60%: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        confirmed_brightness = body_get2.get("brightness")
        line = f"[INFO] Brightness after setting to ~60%: {confirmed_brightness!r} (target={target_brightness!r})"
        LOGGER.result(line)
        logs.append(line)

        if confirmed_brightness != target_brightness:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Expected brightness {target_brightness!r}, observed {confirmed_brightness!r}."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 5: Manual video-content verification
        line = "[STEP] Manual check required — start video playback and validate brightness."
        LOGGER.result(line)
        logs.append(line)

        for line in (
            "[DESC] Start playback of any video content (e.g., YouTube, VOD app).",
            "[DESC] Observe the picture brightness and compare it with the previous brightness level.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        user_validated = yes_or_no(
            result,
            logs,
            "While video content is playing, does the picture clearly reflect the ~60% brightness setting (different from the original level)?",
        )
        if user_validated:
            result.test_result = "PASS"
            line = "[RESULT] PASS — User confirmed video brightness matches the ~60% brightness setting."
        else:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — User reported video brightness does NOT match the ~60% brightness setting."
        ...
        # Final summary log
        line = f"[SUMMARY] outcome={result.test_result}, user_validated_video={user_validated}, test_id={test_id}, device={device_id}"
        LOGGER.result(line)
        logs.append(line)
...
def run_brightness_max_increment_guard_check(dab_topic, test_name, tester, device_id):
    """
    Validates that attempting to set brightness above the maximum supported value
    does not drive the stored brightness beyond its upper limit. Fully automatic
    negative test (no manual confirmation required).

    Flow:
      1) Use EnforcementManager to resolve brightness min/max range.
      2) Read and store the original brightness value.
      3) Set brightness to the maximum value via system/settings/set and confirm.
      4) Send a system/settings/set with brightness above the maximum (max+1).
      5) Accept either:
         - 400 Bad Request (preferred), or
         - 200 OK if the device clamps/ignores but keeps brightness <= max.
      6) Read brightness again and verify it is not above max (ideally == max).
      7) Restore original brightness (best-effort).
    """
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    ...
    try:
        # Header and description
        for line in (
            f"[TEST] Brightness Max Increment Guard Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Verify that attempting to set brightness above the maximum supported value does not exceed that upper limit.",
            "[DESC] Required operations: system/settings/get, system/settings/set.",
            "[DESC] Pass criteria: Device rejects or clamps the above-max value; stored brightness does not exceed the max.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        # Step 1: Resolve brightness range using EnforcementManager (settings/list)
        line = "[STEP] Resolving brightness range using EnforcementManager settings descriptor."
        LOGGER.result(line)
        logs.append(line)
        ...
        if not isinstance(desc, dict) or not {"min", "max"}.issubset(desc.keys()):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness descriptor does not expose numeric min/max; cannot construct above-max value."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        min_brightness = desc["min"]
        max_brightness = desc["max"]
        line = f"[INFO] brightness range from settings/list: min={min_brightness!r}, max={max_brightness!r}"
        LOGGER.result(line)
        logs.append(line)

        if not isinstance(min_brightness, (int, float)) or not isinstance(max_brightness, (int, float)):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness min/max values are not numeric; cannot run max increment guard."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 2: Read current brightness (for restore later)
        line = "[STEP] Fetching current brightness value via system/settings/get."
        LOGGER.result(line)
        logs.append(line)
        ...
        # Step 3: Set brightness to max and confirm
        payload_max = json.dumps({"brightness": max_brightness})
        line = f"[STEP] Setting brightness to its maximum value via system/settings/set: {payload_max}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_max, logs, result)
        status = dab_status_from(response, rc)
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/settings/set for brightness is not implemented on this device."
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/set to max brightness returned status {status}; cannot establish precondition."
            LOGGER.result(line)
            logs.append(line)
            return result

        line = "[STEP] Confirming brightness is at maximum via system/settings/get."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get after setting max brightness failed with status {status}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get returned invalid JSON after setting max brightness: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        confirmed_max = body_get_max.get("brightness")
        line = f"[INFO] Brightness after setting to max: {confirmed_max!r} (expected max={max_brightness!r})"
        LOGGER.result(line)
        logs.append(line)

        if confirmed_max != max_brightness:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Unable to confirm brightness at max. Expected {max_brightness!r}, observed {confirmed_max!r}."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 4: Send above-max brightness (negative)
        invalid_value = max_brightness + 1
        payload_invalid = json.dumps({"brightness": invalid_value})
        line = f"[STEP] Sending above-max brightness value via system/settings/set; max={max_brightness!r}, invalid_value={invalid_value!r}, payload={payload_invalid}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_invalid, logs, result)
        status = dab_status_from(response, rc)

        # Negative-test expectation: 400 preferred; 200 allowed if device clamps; 501 -> OPTIONAL_FAILED; any other -> FAILED.
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/settings/set returned 501 for above-max brightness; cannot validate guard on this device."
            LOGGER.result(line)
            logs.append(line)
            return result
        elif status not in (200, 400):
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Unexpected status for above-max brightness request: got {status}, expected 400 (Bad Request) or 200 (clamp/ignore)."
            LOGGER.result(line)
            logs.append(line)
            # We still continue to read back brightness to see what happened.

        # Step 5: Verify brightness did not exceed max
        line = "[STEP] Reading brightness via system/settings/get after above-max request."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status != 200:
            if result.test_result == "UNKNOWN":
                result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get after above-max brightness request failed with status {status}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            if result.test_result == "UNKNOWN":
                result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get returned invalid JSON after above-max brightness request: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        brightness_after = body_get2.get("brightness")
        line = f"[INFO] Brightness after above-max request: {brightness_after!r} (max={max_brightness!r}, invalid_value={invalid_value!r})"
        LOGGER.result(line)
        logs.append(line)
        ...
        if brightness_after is None:
            if result.test_result == "UNKNOWN":
                result.test_result = "FAILED"
            line = "[RESULT] FAILED — brightness key missing after above-max request; cannot verify guard."
            LOGGER.result(line)
            logs.append(line)
            return result

        if isinstance(brightness_after, (int, float)) and brightness_after <= max_brightness and brightness_after == max_brightness:
            if result.test_result == "UNKNOWN":
                result.test_result = "PASS"
            line = "[RESULT] PASS — Device prevented brightness from exceeding max; value remains at the maximum boundary."
            LOGGER.result(line)
            logs.append(line)
        else:
            if result.test_result == "UNKNOWN":
                result.test_result = "FAILED"
            line = (f"[RESULT] FAILED — Brightness guard failed or behaved unexpectedly. "
                    f"max={max_brightness!r}, invalid_value={invalid_value!r}, observed brightness={brightness_after!r}.")
            LOGGER.result(line)
            logs.append(line)
        ...
        # Final summary log
        line = f"[SUMMARY] outcome={result.test_result}, min={min_brightness!r}, max={max_brightness!r}, test_id={test_id}, device={device_id}"
        LOGGER.result(line)
        logs.append(line)
...
def run_brightness_rapid_change_responsiveness_check(dab_topic, test_name, tester, device_id):
    """
    Validates that brightness control remains responsive when rapidly changing brightness
    between multiple values (approx. 20%, 80%, 40% of the supported range).
    This is a manual-assisted test.
    """
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    result = TestResult(test_id, device_id, "system/settings/set", "{}", "UNKNOWN", "", logs)
    ...
    try:
        # Header and description
        for line in (
            f"[TEST] Brightness Rapid-Change Responsiveness Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Quickly change brightness to ~20%, ~80%, then ~40% and verify the control is responsive.",
            "[DESC] Required operations: system/settings/get, system/settings/set.",
            "[DESC] Pass criteria: Device accepts rapid updates and user confirms screen brightness changes promptly for each step.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        if not isinstance(desc, dict) or not {"min", "max"}.issubset(desc.keys()):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness descriptor does not expose numeric min/max; cannot compute 20/40/80% values."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if not isinstance(min_brightness, (int, float)) or not isinstance(max_brightness, (int, float)):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness min/max values are not numeric; cannot compute percentage-based values."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        line = f"[INFO] Computed brightness values: ~20%={value_20!r}, ~40%={value_40!r}, ~80%={value_80!r}"
        LOGGER.result(line)
        logs.append(line)
        ...
        # Step 3: Rapidly update brightness to 20% then 80% then 40%
        line = "[STEP] Manual check required — observe the screen while brightness is changed quickly to ~20%, ~80%, then ~40%."
        LOGGER.result(line)
        logs.append(line)

        # 3a: Set to ~20%
        payload_20 = json.dumps({"brightness": value_20})
        line = f"[STEP] Setting brightness to ~20% with payload: {payload_20}"
        LOGGER.result(line)
        logs.append(line)
        rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_20, logs, result)
        status = dab_status_from(response, rc)
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/settings/set for brightness is not implemented on this device."
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/set to ~20% returned status {status}; cannot continue rapid-change sequence."
            LOGGER.result(line)
            logs.append(line)
            return result

        # 3b: Set to ~80%
        payload_80 = json.dumps({"brightness": value_80})
        line = f"[STEP] Setting brightness to ~80% with payload: {payload_80}"
        LOGGER.result(line)
        logs.append(line)
        rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_80, logs, result)
        status = dab_status_from(response, rc)
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/set to ~80% returned status {status}; cannot continue rapid-change sequence."
            LOGGER.result(line)
            logs.append(line)
            return result

        # 3c: Set to ~40%
        payload_40 = json.dumps({"brightness": value_40})
        line = f"[STEP] Setting brightness to ~40% with payload: {payload_40}"
        LOGGER.result(line)
        logs.append(line)
        rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_40, logs, result)
        status = dab_status_from(response, rc)
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/set to ~40% returned status {status}; rapid-change sequence did not complete."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 4: Confirm final stored value is ~40%
        line = "[STEP] Confirming final brightness value (~40%) via system/settings/get."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get after rapid changes failed with status {status}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get returned invalid JSON after rapid changes: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        final_brightness = body_get2.get("brightness")
        line = f"[INFO] Brightness after rapid changes: {final_brightness!r} (expected ~40%={value_40!r})"
        LOGGER.result(line)
        logs.append(line)

        if final_brightness != value_40:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Expected final brightness {value_40!r}, observed {final_brightness!r} after rapid changes."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 5: Manual verification of responsiveness
        line = "[STEP] Manual verification — confirm that brightness changes were prompt and visible after each update (20% → 80% → 40%)."
        LOGGER.result(line)
        logs.append(line)
        ...
        user_validated = yes_or_no(
            result,
            logs,
            "While brightness was changed to ~20%, then ~80%, then ~40%, did the screen brightness update promptly after each change?"
        )

        if user_validated:
            result.test_result = "PASS"
            line = "[RESULT] PASS — User confirmed that brightness control remained responsive and screen updated after each change."
        else:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — User reported that brightness control was not responsive or updates were delayed."
        ...
        # Final summary log
        line = f"[SUMMARY] outcome={result.test_result}, user_validated_responsiveness={user_validated}, test_id={test_id}, device={device_id}"
        LOGGER.result(line)
        logs.append(line)
...
def run_brightness_min_value_screen_check(dab_topic, test_name, tester, device_id):
    """
    Validates that setting brightness to the minimum supported value via DAB is stored correctly
    and reflected on the screen. This is a manual-assisted test.
    """
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    ...
    try:
        # Header and description
        for line in (
            f"[TEST] Brightness Minimum Value Screen Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Set brightness to the minimum supported value and verify it is reflected on the screen.",
            "[DESC] Required operations: system/settings/get, system/settings/set.",
            "[DESC] Pass criteria: Device reports minimum brightness via DAB and user confirms very low screen brightness.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        if not isinstance(desc, dict) or not {"min", "max"}.issubset(desc.keys()):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness descriptor does not expose numeric min/max; cannot determine minimum value."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if not isinstance(min_brightness, (int, float)):
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — brightness min value is not numeric; cannot run minimum value screen check."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        # Step 3: Set brightness to minimum
        payload_min = json.dumps({"brightness": min_brightness})
        line = f"[STEP] Setting brightness to minimum value via system/settings/set: {payload_min}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "system/settings/set", payload_min, logs, result)
        status = dab_status_from(response, rc)
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/settings/set for brightness is not implemented on this device."
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/set to minimum returned status {status}; cannot proceed."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 4: Confirm brightness is at minimum via get
        line = "[STEP] Confirming brightness is at minimum via system/settings/get."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get after setting minimum failed with status {status}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/settings/get returned invalid JSON after setting minimum: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        confirmed_brightness = body_get2.get("brightness")
        line = f"[INFO] Brightness after setting to minimum: {confirmed_brightness!r} (expected min={min_brightness!r})"
        LOGGER.result(line)
        logs.append(line)

        if confirmed_brightness != min_brightness:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Expected brightness {min_brightness!r}, observed {confirmed_brightness!r}."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 5: Manual visual confirmation on screen
        line = "[STEP] Manual check required — observe the screen at the minimum brightness setting."
        LOGGER.result(line)
        logs.append(line)

        for line in (
            "[DESC] The screen should now appear very dim / minimally lit compared to normal brightness.",
            "[DESC] Compare this to a higher brightness level (e.g., before running this test) if needed.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        user_validated = yes_or_no(
            result,
            logs,
            "Does the screen appear at a very low / minimum brightness level after setting brightness to the minimum value?"
        )

        if user_validated:
            result.test_result = "PASS"
            line = "[RESULT] PASS — User confirmed the screen reflects the minimum brightness as expected."
        else:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — User reported the screen does NOT reflect the minimum brightness level."
        ...
        # Final summary log
        line = f"[SUMMARY] outcome={result.test_result}, user_validated_min={user_validated}, min={min_brightness!r}, test_id={test_id}, device={device_id}"
        LOGGER.result(line)
        logs.append(line)
...
def run_content_search_partial_app_metadata_check(dab_topic, test_name, tester, device_id):
    ...
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    # Use the concrete topic we care about for this test result
    result = TestResult(test_id, device_id, "content/search", "{}", "UNKNOWN", "", logs)

    # We will try multiple partial keywords and accept PASS if any of them
    # yields valid results with complete metadata.
    ...
    try:
        # Header / description
        for line in (
            f"[TEST] Content Search Partial App Metadata Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Use content/search with partial app-related keywords and verify that results provide complete ContentEntry metadata.",
            "[DESC] Required operation: content/search.",
            "[DESC] Pass criteria: At least one partial keyword returns status=200, a non-empty entries list, and each entry has entryId, appId, title, poster, and categories with correct types.",
        ):
            LOGGER.result(line)
            logs.append(line)

        # Capability gate
        if not require_capabilities(tester, device_id, "ops: content/search", result, logs):
            # require_capabilities sets result.test_result to OPTIONAL_FAILED on failure.
            return result

        # Try each partial keyword until one gives a usable response
        found_valid_entries = False
        for search_text in search_terms:
            used_search_text = search_text
            line = f"[STEP] Sending content/search with partial keyword: {search_text!r}"
            LOGGER.result(line)
            logs.append(line)

            payload = json.dumps({"searchText": search_text})
            rc, response = execute_cmd_and_log(tester, device_id, "content/search", payload, logs, result)
            status = dab_status_from(response, rc)
            line = f"[INFO] content/search returned status={status} for searchText={search_text!r}"
            LOGGER.result(line)
            logs.append(line)

            # 501 → feature not implemented on this device
            if status == 501:
                result.test_result = "OPTIONAL_FAILED"
                line = "[RESULT] OPTIONAL_FAILED — content/search is not implemented on this device (status=501)."
                LOGGER.result(line)
                logs.append(line)
                return result

            # Non-200 but not 501: log and try the next keyword
            if status != 200:
                line = f"[INFO] Skipping searchText={search_text!r} because status={status} (expected 200); trying next keyword if available."
                LOGGER.result(line)
                logs.append(line)
                continue
            ...
            # Parse JSON body
            try:
                body = json.loads(response) if isinstance(response, str) else (response or {})
            except Exception as e:
                result.test_result = "FAILED"
                line = f"[RESULT] FAILED — content/search response for searchText={search_text!r} is not valid JSON: {e}"
                LOGGER.result(line)
                logs.append(line)
                return result
            ...
            if not isinstance(entries, list) or not entries:
                line = f"[INFO] content/search returned an empty or invalid 'entries' list for searchText={search_text!r}; trying next keyword if available."
                LOGGER.result(line)
                logs.append(line)
                continue

            line = f"[INFO] content/search returned {len(entries)} entries for searchText={search_text!r}."
            LOGGER.result(line)
            logs.append(line)

            # Validate metadata for each entry (essential ContentEntry fields)
            required_fields = ("entryId", "appId", "title", "poster", "categories")
            for idx, entry in enumerate(entries):
                if not isinstance(entry, dict):
                    result.test_result = "FAILED"
                    line = f"[RESULT] FAILED — Entry #{idx} is not an object; got type={type(entry).__name__}."
                    LOGGER.result(line)
                    logs.append(line)
                    return result
                ...
                if missing:
                    result.test_result = "FAILED"
                    line = f"[RESULT] FAILED — Entry #{idx} is missing required keys: {missing}."
                    LOGGER.result(line)
                    logs.append(line)
                    return result
                ...
                if not isinstance(entry_id, str) or not entry_id:
                    result.test_result = "FAILED"
                    line = f"[RESULT] FAILED — Entry #{idx} has invalid entryId={entry_id!r} (expected non-empty string)."
                    LOGGER.result(line)
                    logs.append(line)
                    return result
                ...
                if not isinstance(app_id, str) or not app_id:
                    result.test_result = "FAILED"
                    line = f"[RESULT] FAILED — Entry #{idx} has invalid appId={app_id!r} (expected non-empty string)."
                    LOGGER.result(line)
                    logs.append(line)
                    return result
                ...
                if not isinstance(title, str) or not title:
                    result.test_result = "FAILED"
                    line = f"[RESULT] FAILED — Entry #{idx} has invalid title={title!r} (expected non-empty string)."
                    LOGGER.result(line)
                    logs.append(line)
                    return result
                ...
                if not isinstance(poster, str) or not poster:
                    result.test_result = "FAILED"
                    line = f"[RESULT] FAILED — Entry #{idx} has invalid poster value (expected non-empty base64 data URL string)."
                    LOGGER.result(line)
                    logs.append(line)
                    return result
                ...
                if not isinstance(categories, list) or not all(isinstance(c, str) for c in categories):
                    result.test_result = "FAILED"
                    line = f"[RESULT] FAILED — Entry #{idx} has invalid categories={categories!r} (expected list of strings)."
                    LOGGER.result(line)
                    logs.append(line)
                    return result

            # If we reach here, this searchText produced valid entries with complete metadata
            found_valid_entries = True
            break  # No need to try remaining search terms

        # Decide final outcome based on whether any searchText passed
        if not found_valid_entries:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — No partial keyword (Netf/Youtu) produced a usable content/search response with non-empty, valid entries."
            LOGGER.result(line)
            logs.append(line)
        else:
            result.test_result = "PASS"
            line = (f"[RESULT] PASS — content/search returned non-empty entries with complete ContentEntry metadata "
                    f"for partial keyword {used_search_text!r}.")
            LOGGER.result(line)
            logs.append(line)

    except UnsupportedOperationError as e:
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' not supported while running content search metadata check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — Internal error during content search metadata check: {e}"
        LOGGER.result(line)
        logs.append(line)

    finally:
        summary_line = (f"[SUMMARY] outcome={result.test_result}, last_searchText={used_search_text!r}, "
                        f"test_id={test_id}, device={device_id}")
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_content_open_invalid_content_id_check(dab_topic, test_name, tester, device_id):
    ...
    # A clearly invalid contentId that should never exist on real devices.
    invalid_content_id = "dab-invalid-content-id-0000-should-not-exist"

    try:
        # Header / description
        for line in (
            f"[TEST] Content_Open Invalid Content_ID Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Ensure content/open rejects a non-existent contentId with an appropriate client error.",
            "[DESC] Required operation: content/open.",
            "[DESC] Pass criteria: content/open returns a 400-style client error for the invalid contentId (not 200), "
            "and response is well-formed JSON if provided.",
        ):
            LOGGER.result(line)
            logs.append(line)

        # Capability gate
        if not require_capabilities(tester, device_id, "ops: content/open", result, logs):
            return result

        # Step 1: Send content/open with an invalid contentId
        payload = json.dumps({"contentId": invalid_content_id})
        line = f"[STEP] Sending content/open with invalid contentId={invalid_content_id!r}, payload={payload}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "content/open", payload, logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] content/open returned status={status} for invalid contentId."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — content/open is not implemented on this device (status=501)."
            LOGGER.result(line)
            logs.append(line)
            return result

        # 200 → incorrect behavior for invalid contentId
        if status == 200:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — content/open returned status=200 for an invalid/non-existent contentId; expected client error."
            LOGGER.result(line)
            logs.append(line)
            return result

        # For your negative test conventions, 400 is the primary expected error code.
        if status != 400:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Unexpected status for invalid contentId: got {status}, expected 400 (Bad Request)."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Optional: parse and inspect error payload if present
        body = None
        if response:
            try:
                body = json.loads(response) if isinstance(response, str) else (response or {})
            except Exception as e:
                # We treat invalid JSON as a failure because a proper DAB error should be structured.
                result.test_result = "FAILED"
                line = f"[RESULT] FAILED — content/open error response for invalid contentId is not valid JSON: {e}"
                LOGGER.result(line)
                logs.append(line)
                return result

        if isinstance(body, dict):
            # Log any error-related fields for debugging; do not enforce exact text.
            error_fields = []
            for key in ("error", "message", "statusMessage", "reason"):
                if key in body:
                    error_fields.append(f"{key}={body.get(key)!r}")
            if error_fields:
                line = "[INFO] content/open error payload fields: " + ", ".join(error_fields)
                LOGGER.result(line)
                logs.append(line)
            else:
                line = "[INFO] content/open error payload does not expose explicit error/message fields; relying on status=400 only."
                LOGGER.result(line)
                logs.append(line)
        else:
            line = "[INFO] content/open returned status=400 with no structured JSON body; treating as a valid client error."
            LOGGER.result(line)
            logs.append(line)

        # If we reached here, the device correctly rejected the invalid contentId
        result.test_result = "PASS"
        line = "[RESULT] PASS — content/open correctly returned a client error (400) for a non-existent contentId."
        LOGGER.result(line)
        logs.append(line)

    except UnsupportedOperationError as e:
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' is not supported while running content/open invalid contentId check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — An unexpected error occurred during content/open invalid contentId check: {e}"
        LOGGER.result(line)
        logs.append(line)

    finally:
        summary_line = (
            f"[SUMMARY] outcome={result.test_result}, invalid_content_id={invalid_content_id!r}, "
            f"test_id={test_id}, device={device_id}"
        )
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_content_recommendations_update_after_watch_check(dab_topic, test_name, tester, device_id):
    ...
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    result = TestResult(test_id, device_id, "content/recommendations", "{}", "UNKNOWN", "", logs)
    user_validated = "N/A"
    ...
    try:
        # Header / description
        for line in (
            f"[TEST] Content_Recommendations Update_After_Watch Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Verify that content/recommendations reflects new viewing activity after watching new content.",
            "[DESC] Required operation: content/recommendations.",
            "[DESC] Pass criteria: Second recommendations call appears updated (content and/or ordering) and user confirms the change.",
        ):
            LOGGER.result(line)
            logs.append(line)

        # Capability gate
        if not require_capabilities(tester, device_id, "ops: content/recommendations", result, logs):
            return result

        # Step 1: Baseline recommendations
        line = "[STEP] Fetching baseline recommendations via content/recommendations."
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "content/recommendations", "{}", logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] Baseline content/recommendations returned status={status}."
        LOGGER.result(line)
        logs.append(line)

        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — content/recommendations is not implemented on this device (status=501)."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Baseline content/recommendations returned non-200 status={status}."
            LOGGER.result(line)
            logs.append(line)
            return result

        try:
            body_initial = json.loads(response) if isinstance(response, str) else (response or {})
        except Exception as e:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Baseline content/recommendations response is not valid JSON: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if not isinstance(initial_entries, list):
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — Baseline content/recommendations 'entries' is not a list."
            LOGGER.result(line)
            logs.append(line)
            return result

        line = f"[INFO] Baseline recommendations count={len(initial_entries)}."
        LOGGER.result(line)
        logs.append(line)
        ...
        for i in range(sample_count):
            ...
            line = f"[INFO] Baseline entry[{i}]: appId={entry.get('appId')!r}, entryId={entry.get('entryId')!r}, title={entry.get('title')!r}"
            LOGGER.result(line)
            logs.append(line)

        if not initial_entries:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — Baseline recommendations list is empty; cannot evaluate update behavior."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 2: Ask user to watch new content
        for line in (
            "[STEP] Manual action required — On the device, watch a new movie/TV show that is not part of the existing viewing history.",
            "[DESC] Preferably choose content that is clearly different from your usual recommendations (e.g., different genre/app).",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        user_ready = yes_or_no(
            result,
            logs,
            "Have you watched the new content (at least a few minutes) and returned to the main/home screen so recommendations can refresh?"
        )
        if not user_ready:
            result.test_result = "SKIPPED"
            line = "[RESULT] SKIPPED — User did not complete the viewing step; cannot validate recommendations update."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 3: Fetch recommendations again
        line = "[STEP] Fetching updated recommendations via content/recommendations after new viewing."
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "content/recommendations", "{}", logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] Updated content/recommendations returned status={status}."
        LOGGER.result(line)
        logs.append(line)

        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Updated content/recommendations returned non-200 status={status}."
            LOGGER.result(line)
            logs.append(line)
            return result

        try:
            body_updated = json.loads(response) if isinstance(response, str) else (response or {})
        except Exception as e:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Updated content/recommendations response is not valid JSON: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if not isinstance(updated_entries, list):
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — Updated content/recommendations 'entries' is not a list."
            LOGGER.result(line)
            logs.append(line)
            return result

        line = f"[INFO] Updated recommendations count={len(updated_entries)}."
        LOGGER.result(line)
        logs.append(line)
        ...
        for i in range(sample_count):
            ...
            line = f"[INFO] Updated entry[{i}]: appId={entry.get('appId')!r}, entryId={entry.get('entryId')!r}, title={entry.get('title')!r}"
            LOGGER.result(line)
            logs.append(line)

        if not updated_entries:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — Updated recommendations list is empty; cannot verify updates."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 4: Basic structural comparison (entryId+appId) to highlight changes
        initial_sig = set()
        ...
        line = f"[INFO] Structural diff: new_items_count={len(new_items)}, removed_items_count={len(removed_items)}."
        LOGGER.result(line)
        logs.append(line)

        if new_items:
            preview = list(new_items)[:5]
            line = f"[INFO] Example new recommendation signatures after viewing: {preview}"
            LOGGER.result(line)
            logs.append(line)
        else:
            line = "[INFO] No new recommendation signatures detected; updates might be ranking-only or delayed."
            LOGGER.result(line)
            logs.append(line)

        # Step 5: Manual confirmation about whether recommendations look updated
        user_validated = yes_or_no(
            result,
            logs,
            "Comparing the recommendations shown on the device UI before and after watching, do they appear updated to reflect the new content you watched?"
        )

        if user_validated:
            result.test_result = "PASS"
            line = "[RESULT] PASS — User confirmed recommendations appear updated after watching new content."
        else:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — User reported that recommendations did NOT appear updated after watching new content."
        ...
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' is not supported while running content/recommendations update check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — An unexpected error occurred during content/recommendations update check: {e}"
        LOGGER.result(line)
        logs.append(line)

    finally:
        summary_line = (
            f"[SUMMARY] outcome={result.test_result}, baseline_count={len(initial_entries)}, "
            f"updated_count={len(updated_entries)}, test_id={test_id}, device={device_id}"
        )
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_content_search_inception_metadata_check(dab_topic, test_name, tester, device_id):
    ...
    try:
        # Header / description
        for line in (
            f"[TEST] Content_Search Inception_Metadata Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Ensure content/search with title 'Inception' returns relevant results with complete metadata.",
            "[DESC] Required operation: content/search.",
            "[DESC] Pass criteria: status=200, non-empty entries with full ContentEntry metadata, "
            "at least one entry title containing 'Inception', and user confirmation that UI shows relevant results.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        # Step 1: Send content/search for "Inception"
        payload = json.dumps({"searchText": search_text})
        line = f"[STEP] Sending content/search with movie title searchText={search_text!r}, payload={payload}"
        LOGGER.result(line)
        logs.append(line)
        ...
        # Positive test expects 200
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — content/search returned non-200 status={status} for searchText={search_text!r}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — content/search response is not valid JSON for searchText={search_text!r}: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if not isinstance(entries, list) or not entries:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — content/search 'entries' is empty or invalid for searchText={search_text!r}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        for idx in range(sample_count):
            ...
            line = f"[INFO] Entry[{idx}]: appId={entry.get('appId')!r}, entryId={entry.get('entryId')!r}, title={entry.get('title')!r}"
            LOGGER.result(line)
            logs.append(line)

        # Step 3: Validate essential metadata for each entry
        required_fields = ("entryId", "appId", "title", "poster", "categories")
        inception_matches = 0
        ...
        for idx, entry in enumerate(entries):
            ...
            # Check if this entry looks like "Inception"
            if "inception" in title.lower():
                inception_matches += 1

        line = f"[INFO] Number of entries whose title contains 'Inception': {inception_matches}"
        LOGGER.result(line)
        logs.append(line)

        if inception_matches == 0:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — No entry title contains 'Inception'; results may not be relevant for this test’s expectations."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 4: Manual UI confirmation
        for line in (
            "[STEP] Manual check required — On the device UI, search for 'Inception' and inspect the displayed results.",
            "[DESC] The on-screen results should be consistent with the DAB response (e.g., show 'Inception' from the expected app).",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        if user_validated:
            result.test_result = "PASS"
            line = "[RESULT] PASS — User confirmed that on-screen search results for 'Inception' are relevant and that JSON metadata is complete."
        else:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — User reported that on-screen search results for 'Inception' are not relevant or inconsistent with DAB response."
        ...
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' is not supported while running content/search Inception check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — An unexpected error occurred during content/search Inception check: {e}"
        LOGGER.result(line)
        logs.append(line)

    finally:
        summary_line = (
            f"[SUMMARY] outcome={result.test_result}, searchText={search_text!r}, "
            f"test_id={test_id}, device={device_id}"
        )
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_system_power_mode_active_to_standby_check(dab_topic, test_name, tester, device_id):
    ...
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    result = TestResult(test_id, device_id, "system/power-mode/set", "{}", "UNKNOWN", "", logs)
    ...
    try:
        # Header / description
        for line in (
            f"[TEST] System_PowerMode Active_To_Standby Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Use system/power-mode/set to move the device from 'Active' to 'Standby', "
            "then verify via system/power-mode/get that the mode is 'Standby' and DAB remains responsive.",
            "[DESC] Required operations: system/power-mode/get, system/power-mode/set.",
            "[DESC] Pass criteria: mode successfully becomes 'Standby' (status=200), and a subsequent get succeeds.",
        ):
            LOGGER.result(line)
            logs.append(line)

        # Capability gate
        if not require_capabilities(tester, device_id, "ops: system/power-mode/get, system/power-mode/set", result, logs):
            return result

        # Step 0: Read current power mode and record original_mode
        line = "[STEP] Reading current power mode via system/power-mode/get."
        LOGGER.result(line)
        logs.append(line)
        rc, response = execute_cmd_and_log(tester, device_id, "system/power-mode/get", "{}", logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] Initial system/power-mode/get returned status={status}."
        LOGGER.result(line)
        logs.append(line)

        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/power-mode/get is not implemented on this device (status=501)."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/power-mode/get did not return 200 initially; got {status}."
            LOGGER.result(line)
            logs.append(line)
            return result

        try:
            body_init = json.loads(response) if isinstance(response, str) else (response or {})
        except Exception as e:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Initial system/power-mode/get response is not valid JSON: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        original_mode = body_init.get("mode")
        line = f"[INFO] Original power mode reported by device: {original_mode!r}"
        LOGGER.result(line)
        logs.append(line)
        ...
        if original_mode != "Active":
            payload_active = json.dumps({"mode": "Active"})
            line = f"[STEP] Precondition: setting power mode to 'Active' via system/power-mode/set, payload={payload_active}"
            LOGGER.result(line)
            logs.append(line)
            rc, response = execute_cmd_and_log(tester, device_id, "system/power-mode/set", payload_active, logs, result)
            status = dab_status_from(response, rc)
            line = f"[INFO] system/power-mode/set('Active') returned status={status}."
            LOGGER.result(line)
            logs.append(line)

            if status == 501:
                result.test_result = "OPTIONAL_FAILED"
                line = "[RESULT] OPTIONAL_FAILED — system/power-mode/set is not implemented on this device (status=501)."
                LOGGER.result(line)
                logs.append(line)
                return result
            ...
            if status != 200:
                result.test_result = "FAILED"
                line = f"[RESULT] FAILED — Could not set power mode to 'Active'; expected status=200, got {status}."
                LOGGER.result(line)
                logs.append(line)
                return result

            line = "[STEP] Confirming power mode is 'Active' via system/power-mode/get."
            LOGGER.result(line)
            logs.append(line)
            rc, response = execute_cmd_and_log(tester, device_id, "system/power-mode/get", "{}", logs, result)
            status = dab_status_from(response, rc)
            line = f"[INFO] system/power-mode/get after setting 'Active' returned status={status}."
            LOGGER.result(line)
            logs.append(line)

            if status != 200:
                result.test_result = "FAILED"
                line = f"[RESULT] FAILED — system/power-mode/get did not return 200 after setting 'Active'; got {status}."
                LOGGER.result(line)
                logs.append(line)
                return result

            try:
                body_active = json.loads(response) if isinstance(response, str) else (response or {})
            except Exception as e:
                result.test_result = "FAILED"
                line = f"[RESULT] FAILED — JSON parsing failed after setting 'Active': {e}"
                LOGGER.result(line)
                logs.append(line)
                return result

            mode_after_active = body_active.get("mode")
            line = f"[INFO] Power mode after setting 'Active': {mode_after_active!r}"
            LOGGER.result(line)
            logs.append(line)

            if mode_after_active != "Active":
                result.test_result = "FAILED"
                line = f"[RESULT] FAILED — Expected power mode 'Active' as precondition, but got {mode_after_active!r}."
                LOGGER.result(line)
                logs.append(line)
                return result
        ...
        # Step 2: Set power mode to 'Standby'
        payload_standby = json.dumps({"mode": "Standby"})
        line = f"[STEP] Setting power mode to 'Standby' via system/power-mode/set, payload={payload_standby}"
        LOGGER.result(line)
        logs.append(line)
        rc, response = execute_cmd_and_log(tester, device_id, "system/power-mode/set", payload_standby, logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] system/power-mode/set('Standby') returned status={status}."
        LOGGER.result(line)
        logs.append(line)
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/power-mode/set('Standby') did not return 200; got {status}."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 3: Confirm power mode is 'Standby' and DAB is still active
        line = "[STEP] Confirming power mode is 'Standby' via system/power-mode/get (also validates DAB remains active)."
        LOGGER.result(line)
        logs.append(line)
        rc, response = execute_cmd_and_log(tester, device_id, "system/power-mode/get", "{}", logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] system/power-mode/get after setting 'Standby' returned status={status}."
        LOGGER.result(line)
        logs.append(line)

        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/power-mode/get did not return 200 after setting 'Standby'; DAB may not be responsive. Got {status}."
            LOGGER.result(line)
            logs.append(line)
            return result

        try:
            body_final = json.loads(response) if isinstance(response, str) else (response or {})
        except Exception as e:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — JSON parsing failed after setting 'Standby': {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        final_mode = body_final.get("mode")
        line = f"[INFO] Power mode after setting 'Standby': {final_mode!r}"
        LOGGER.result(line)
        logs.append(line)
        ...
        if final_mode != "Standby":
            result.test_result = "FAILED"
            line = (
                f"[RESULT] FAILED — Expected power mode 'Standby', but got {final_mode!r}. "
                "DAB is responsive, but mode did not transition as expected."
            )
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        # All checks passed
        result.test_result = "PASS"
        line = "[RESULT] PASS — Device transitioned from 'Active' to 'Standby', and DAB remained active with mode confirmed as 'Standby'."
        LOGGER.result(line)
        logs.append(line)

    except UnsupportedOperationError as e:
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' is not supported while running Active→Standby power mode check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — Unexpected error during Active→Standby power mode check: {e}"
        LOGGER.result(line)
        logs.append(line)
        ...
        try:
            if original_mode is not None and original_mode != "Standby":
                ...
                line = f"[STEP] Best-effort restore: setting power mode back to original value {original_mode!r}."
                LOGGER.result(line)
                logs.append(line)
                rc_restore, _ = execute_cmd_and_log(tester, device_id, "system/power-mode/set", payload_restore, logs, result)
                line = f"[INFO] Restore system/power-mode/set returned rc={rc_restore}."
                LOGGER.result(line)
                logs.append(line)
        except Exception as e:
        ...
        summary_line = f"[SUMMARY] outcome={result.test_result}, original_mode={original_mode!r}, final_mode={final_mode!r}, test_id={test_id}, device={device_id}"
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
FUNCTIONAL_TEST_CASE = [
    ...
    ("system/settings/set", "functional", run_contrast_minimum_value_check, "Contrast Minimum Value Check", "2.1", False),
    ("system/settings/set", "functional", run_contrast_invalid_value_check, "Contrast Invalid Value Check", "2.1", True),
    ("system/settings/set", "functional", run_brightness_min_decrement_guard_check, "Brightness Min Decrement Guard Check", "2.1", True),
    ("system/settings/set", "functional", run_brightness_mid_level_50_screen_check, "Brightness Mid-Level 50 Screen Check", "2.1", False),
    ("system/settings/set", "functional", run_brightness_60_video_content_check, "Brightness 60 Video_Content Check", "2.1", False),
    ("system/settings/set", "functional", run_brightness_max_increment_guard_check, "Brightness Max Increment Guard Check", "2.1", True),
    ("system/settings/set", "functional", run_brightness_rapid_change_responsiveness_check, "Brightness Rapid_Change Responsiveness Check", "2.1", False),
    ("system/settings/set", "functional", run_brightness_min_value_screen_check, "Brightness Minimum_Value Screen Check", "2.1", False),
    ("content/search", "functional", run_content_search_partial_app_metadata_check, "Content_Search Partial_App Metadata Check", "2.1", False),
    ("content/open", "functional", run_content_open_invalid_content_id_check, "Content_Open Invalid_Content_ID Check", "2.1", True),
    ("content/recommendations", "functional", run_content_recommendations_update_after_watch_check, "Content_Recommendations Update_After_Watch Check", "2.1", False),
    ("content/search", "functional", run_content_search_inception_metadata_check, "Content_Search Inception_Metadata Check", "2.1", False),
    ("system/power-mode/set", "functional", run_system_power_mode_active_to_standby_check, "System_PowerMode Active_To_Standby Check", "2.1", False),
